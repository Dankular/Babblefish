"""
Real-time Speech Translation Server with TTS
Features:
- WebSocket streaming
- VAD-based segmentation
- Whisper transcription
- NLLB translation
- Kokoro TTS (default for English)
- F5-TTS speaker training (passive)
"""
...
# Existing VAD and Buffer classes (copy from realtime_server.py)
class SileroVAD:
...
class RealtimeTranslationHandler:
    """Handle real-time transcription, translation, and TTS"""
    ...
    async def handle_audio_stream(
        ...
        # Check if trained voice is available
        training_stats = self.tts.get_training_stats()
        can_use_trained = training_stats['ready_to_train'] and use_trained_voice
            ...
            while True:
                ...
                if segment is not None and len(segment) > 0:
                    ...
                    if transcription.strip():
                        # Add to speaker training (passive learning)
                        self.tts.add_training_sample(
                            segment,
                            transcription,
                            detected_lang,
                        ...
                        # Generate TTS (English output only)
                        tts_audio_b64 = None
                        ...
                        result = {
                            ...
                            "tts_voice": "trained" if can_use_trained else "kokoro",
...
async def startup():
    ...
    print("Loading models for real-time translation with TTS...")
    ...
    realtime_handler = RealtimeTranslationHandler(whisper_model, nllb_translator, tts_engine)

    print("[OK] Real-time translation with TTS ready!")
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    await websocket.send_json({
        "type": "ready",
        "message": "Ready for audio stream with TTS",
        "target_lang": target_lang,
...
async def get_client():
    ...
    try:
        html_content = open("realtime_client_tts.html", "r", encoding="utf-8").read()
        return HTMLResponse(content=html_content)
    except:
        return HTMLResponse(content="<h1>BabbleFish TTS Server</h1><p>Client not found. Use realtime_client_tts.html</p>")
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run(app, host="0.0.0.0", port=8002)  # Port 8002 for TTS version
